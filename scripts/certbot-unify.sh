...
for domain in `ls ${path_live}/*/ -d 2>/dev/null`; do
	...
	cat ${path_live}/${domain}/fullchain.pem ${path_live}/${domain}/privkey.pem > ${path_ssl}/haproxy/private/${domain}.pem
done

# generate list of pem files
ls -1 ${path_ssl}/haproxy/private/*.pem > ${path_ssl}/certbot.pem.lst
